    ...
    <div>
      ...
      <div className="p-8">
        ...
        {!loading && !error && propiedades.length === 0 && (
          <div className="text-center py-20">
            <p className="text-zinc-600 font-mono text-sm uppercase tracking-widest mb-4">Sin propiedades registradas</p>
            <Button onClick={() => navigate('/propiedades/nueva')}>+ Registrar primera propiedad</Button>
          </div>
        )}
        ...
        {!loading && propiedades.length > 0 && (
          <div className="grid gap-4">
            {propiedades.map((p) => (
              <div key={p.id} className="bg-zinc-900 border border-zinc-800 hover:border-zinc-600 transition-colors p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {p.info_adicional && (
                      <p className="text-zinc-600 font-mono text-xs mt-1 truncate">{p.info_adicional}</p>
                    )}
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    <Button size="sm" variant="ghost" onClick={() => navigate(`/propiedades/${p.id}/editar`)}>
                      Editar
                    </Button>
                    <Button size="sm" variant="secondary" onClick={() => navigate(`/revisiones/nueva?propiedadId=${p.id}`)}>
                      + Revisión
                    </Button>
      ...
      <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Confirmar eliminación">
        <p className="text-zinc-300 text-sm font-mono mb-6">
          ¿Eliminar la propiedad en <span className="text-amber-400">{deleteTarget?.direccion}</span>? Esta acción es irreversible.
        </p>
